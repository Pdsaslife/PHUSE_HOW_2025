...
/*png(file="/nfsshare/sashls/custdata/PHUSE_HOW_2025/boxplot.png",
width=600, height=350)*/

PROC IML;
call ExportDataSetToR ("Work.teams","teams");

submit / R;

tapply(teams$Score, teams$Outcome, summary)

library(ggplot2)

p <- ggplot(teams, aes(x=Outcome, y=Score, color=Outcome)) + geom_boxplot() + labs(title="Superbowl Scores by Outcome")

ggsave("/nfsshare/sashls/custdata/PHUSE_HOW_2025/boxplot.png", plot = p, width = 8, height = 6, units = "in", dpi = 300)
p
dev.off()
...
/* Show the graph into R  */
proc template;
	define statgraph plottemp;
		begingraph;
		layout overlay;
		drawimage "/nfsshare/sashls/custdata/PHUSE_HOW_2025/boxplot.png" / scale=fit;
		endlayout;
		endgraph;
	end;
run;
...
proc sgrender template=plottemp; 
run;
